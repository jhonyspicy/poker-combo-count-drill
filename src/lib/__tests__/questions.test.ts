...
import {
  ADVANCED_CHOICE_BUCKETS, advancedBucketIndex, HERO_RANGE_ID,
...
describe('generateQuestion: 上級のバケット4択', () => {
  it('選択肢は4帯の昇順固定（シャッフルされない）', () => {
    for (let i = 0; i < 20; i++) {
      const q = generateQuestion('advanced');
      expect(q.choices.map(c => c.label)).toEqual(
        ADVANCED_CHOICE_BUCKETS.map(b => b.label)
      );
    }
  });

  it('初期値の境界（10/40/80）から導出したラベルになる', () => {
    const q = generateQuestion('advanced');
    expect(q.choices.map(c => c.label)).toEqual(['0〜10', '11〜40', '41〜80', '81以上']);
  });

  it('正解フラグは答え（負けコンボ数）が属する帯にのみ立つ', () => {
    for (let i = 0; i < 30; i++) {
      const q = generateQuestion('advanced');
      q.choices.forEach((c, index) => {
        expect(c.correct).toBe(index === advancedBucketIndex(q.answer));
      });
    }
  });

  it('正解の帯は特定の帯に固定されない（目標バケット再抽選の検証）', () => {
    const seen = new Set<number>();
    for (let i = 0; i < 60 && seen.size < 2; i++) {
      const q = generateQuestion('advanced');
      seen.add(advancedBucketIndex(q.answer));
    }
